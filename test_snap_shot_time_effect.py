...
class Net(nn.Module):
    def __init__(self):
        ...
        self.fc0 = nn.Linear(inputsDim, units)
...
time_effect=np.zeros(seqLen)
for itr in range(seqLen):
    net = Net()
    ...
    if not trained:
        ...
        time_effect[itr]=max(accuracies_validate)
...
plt.figure()
plt.plot(np.arange(-3, -2, 0.1), time_effect)
