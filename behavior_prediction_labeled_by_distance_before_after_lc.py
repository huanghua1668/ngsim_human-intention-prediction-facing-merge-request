...
def reLabel(data, y):
    sz=int(data.shape[0]/seqLen)
    for i in range(sz):
        if data[i*seqLen,0]==1:
            continue # for merge after
        dx0=data[i*seqLen,5]
        dx1=data[i*seqLen+seqLen,5]
        if dx1>=dx0:
            y[i*seqLen:i*seqLen+seqLen,-1]=1.
        else:
            y[i*seqLen:i*seqLen+seqLen,-1]=0.
    return y
