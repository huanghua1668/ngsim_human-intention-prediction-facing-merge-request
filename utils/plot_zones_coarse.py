...
for i in range(data.shape[0]):
    dv=data[i, dvInd]
    vInd=int((dv-(vMin-deltaV/2.))/deltaV)
    dx=data[i, dxInd]
    xInd=int((dx-(xMin-deltaX/2.))/deltaX)
    if vInd>=0 and vInd<nv and xInd>=0 and xInd<nx:
        scores[vInd, xInd, 0]+=predict[i]
        #scores[vInd, xInd, 0]+=predict_prob[i]
        scores[vInd, xInd, 1]+=1
...
x=np.arange(vMin, vMax+deltaV, deltaV) 
y=np.arange(xMin, xMax+deltaX, deltaX)
x,y=np.meshgrid(x,y)
...
cs=ax.contour(x,y,z, np.linspace(0,1,10))
