...
def pre_train(model, optimizer, dl_train, dl_test, x_train, y_train, run, maxEpoch=200):
    ...
    for epoch in range(maxEpoch):
        ...
        if accuracy > bestValidationAcc:
            ...
            PATH = '/home/hh/data/csnn_4_layers_run{}_epoch{}.pth'.format(run, 0)
    ...
    plot_save_loss(losses, losses_validate, '/home/hh/data/loss_csnn_pretrain_4_layers_run{}.png'.format(run))
    plot_save_acc(accuracies, accuracies_validate, '/home/hh/data/acc_csnn_pretrain_4_layers_run{}.png'.format(run))
...
if __name__ == "__main__":
    ...
    if not trained:
        ...
        for run in range(runs):
            ...
            for epoch in range(epochs):
                ...
                if accuracy > bestValidationAcc:
                    # stateDict = model.state_dict()
                    # include both learnable param and registered buffer
                    PATH = '/home/hh/data/csnn_4layers_run{}.pth'.format(run)
                    torch.save({'net':model, 'alpha':alpha, 'r2':r2}, PATH)
                    bestValidationAcc = accuracy

                if (   (epoch<200 and (epoch + 1) % (epochs / 100) == 0)
                        or (epoch>=200 and (epoch + 1) % (epochs/10) == 0)):
                    PATH = '/home/hh/data/csnn_4_layers_run{}_epoch{}.pth'.format(run, epoch + 1)
                    torch.save({'net': model, 'alpha': alpha, 'r2': r2}, PATH)
            ...
            dir = '/home/hh/data/loss_csnn_4_layers'
            np.savez(dir + ".npz", a=np.array(losses), b=np.array(losses_validate), c=np.array(mmcs))
            dir = '/home/hh/data/acc_csnn_4_layers'
            np.savez(dir + ".npz", a=np.array(accuracies), b=np.array(accuracies_validate))
            dir = '/home/hh/data/csnn_nz_mmcs_4_layers.npz'
            np.savez(dir, a=np.array(nzs), b=np.array(mmcs))

            plot_save_loss(losses, losses_validate, '/home/hh/data/loss_csnn_4_layers.png')
            plot_save_acc(accuracies, accuracies_validate, '/home/hh/data/acc_csnn_4_layers.png')
            plot_save_acc_nzs_mmcs(alphas, accuracies_validate, nzs, mmcs, '/home/hh/data/acc_nzs_mmcs_csnn_4_layers.png')
        ...
        for i in range(200, 1001, 100):
            epochs.append(i)
        alphas = np.array(epochs)/1000.
        for run in range(runs):
            aucs = []
            accs= []
            for epoch in epochs:
                PATH = '/home/hh/data/csnn_4_layers_run{}_epoch{}.pth'.format(run, epoch)
                if epoch==0:
                    PATH = '/home/hh/data/csnn_4_layers_run{}_epoch{}.pth'.format(run, epoch)
                l = torch.load(PATH)
                model = l['net']
                ...
                dir = '/home/hh/data/roc_csnn_4_layers_run{}_epoch{}.png'.format(run, epoch)
