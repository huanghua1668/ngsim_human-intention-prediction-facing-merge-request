    ...
    if not trained:
        ...
        for run in range(runs):
            ...
            for epoch in range(epochs):
                ...
                if accuracy > bestValidationAcc:
                    ...
                    PATH = '/home/hh/data/csnn_4layers_batchNorm_maxAlpha{:.2f}_run{}.pth'.format(maxAlpha, run)
                    torch.save({'net':model, 'alpha':alpha, 'r2':r2}, PATH)
                    bestValidationAcc = accuracy
                ...
                if (   (epoch<200 and (epoch + 1) % (epochs / 100) == 0)
                    ...
                    PATH = '/home/hh/data/csnn_4_layers_batchNorm_maxAlpha{:.2f}_run{}_epoch{}.pth'.format(maxAlpha, run, epoch + 1)
                    torch.save({'net': model, 'alpha': alpha, 'r2': r2}, PATH)
            ...
            # print('epoch ', epoch)
            dir0 = '/home/hh/data/'
            dir = '/home/hh/data/loss_csnn_4_layers_batchNorm_maxAlpha{:.2f}_run{}'.format(maxAlpha, run)
            np.savez(dir + ".npz", a=np.array(losses), b=np.array(losses_validate), c=np.array(mmcs))
            dir = '/home/hh/data/acc_csnn_4_layers_batchNorm_maxAlpha{:.2f}_run{}'.format(maxAlpha, run)
            np.savez(dir + ".npz", a=np.array(accuracies), b=np.array(accuracies_validate))
            dir = '/home/hh/data/csnn_nz_mmcs_4_layers_batchNorm_maxAlpha{:.2f}_run{}.npz'.format(maxAlpha, run)
            np.savez(dir, a=np.array(nzs), b=np.array(mmcs))

            plot_save_loss(losses, losses_validate,
                           '/home/hh/data/loss_csnn_4_layers_batchNorm_maxAlpha{:.2f}_run{}.png'.format(maxAlpha, run))
            plot_save_acc(accuracies, accuracies_validate,
                           '/home/hh/data/acc_csnn_4_layers_batchNorm_maxAlpha{:.2f}_run{}.png'.format(maxAlpha, run))
            plot_save_acc_nzs_mmcs(alphas, accuracies_validate, nzs, mmcs,
                           '/home/hh/data/acc_nzs_mmcs_csnn_4_layers_batchNorm_maxAlpha{:.2f}_run{}.png'.format(maxAlpha, run))
        ...
        for run in range(runs):
            ...
            for epoch in epochs:
                ...
                PATH = '/home/hh/data/csnn_4_layers_batchNorm_maxAlpha{:.2f}_run{}_epoch{}.pth'.format(maxAlpha, run, epoch)
                if epoch==0:
                    PATH = '/home/hh/data/csnn_4_layers_batchNorm_run{}_epoch{}.pth'.format(run, epoch)
                l = torch.load(PATH)
                model = l['net']
                ...
                dir = '/home/hh/data/roc_csnn_4_layers_batchNorm_maxAlpha{:.2f}_run{}_epoch{}.png'.format(maxAlpha, run, epoch)
        ...
        dir = '/home/hh/data/mean_std_accs_aucs_csnn_maxAlpha{:.2f}_4_layers_batchNorm.npz'.format(maxAlpha)
